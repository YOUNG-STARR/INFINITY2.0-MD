...
// Exemple d'utilisation dans une réaction automatique :
function getRandomAutoReactEmoji() {
    return AUTO_REACT_EMOJIS[Math.floor(Math.random() * AUTO_REACT_EMOJIS.length)];
}


process.on('uncaughtException', (err) => {
    console.error(" Erreur non intercepte :", err);
});

process.on('unhandledRejection', (reason, promise) => {
    console.error(" Rejection non gre :", reason);
});
...
"use strict";
    ...
    let evt = require(__dirname + "/framework/infinityy");
    ...
    var session = conf.session.replace(/INFINITY-MD-BOT;;;=>/g,"");
    ...
    const emojis_reaction = ['', '', '', '', '', '', '', '', '', '', '', ''];
            ...
            const sockOptions = {
                ...
                browser: ['Infinity-Md', "safari", "1.0.0"],
            ...
            sock.ev.on("messages.upsert", async (m) => {
              ...
              //  console.log(ms) ;
                ...
                var idBot = decodeJid(sock.user.id);
                var servBot = idBot.split('@')[0];
                /* const dj='50939103464';
                 const dj2='50940525832';
                 const luffy='50947615862'*/
                /*  var superUser=[servBot,dj,dj2,luffy].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
                  var dev =[dj,dj2,luffy].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
                const verifGroupe = origineMessage?.endsWith("@g.us");
                var infosGroupe = verifGroupe ? await sock.groupMetadata(origineMessage) : "";
                ...
                function repondre(mes) { sock.sendMessage(origineMessage, { text: mes }, { quoted: ms }); }
                console.log("\t [][]...{Infinity-Md}...[][]");
    ...
    function mybotpic() {
      // Gnrer un indice alatoire entre 0 (inclus) et la longueur du tableau (exclus)
      const indiceAleatoire = Math.floor(Math.random() * liens.length);
      // Rcuprer le lien correspondant  l'indice alatoire
      ...
      return lienAleatoire;
        ...
        if(ms.key.fromMe || ms.message.protocolMessage.key.fromMe) { console.log('Message supprimer me concernant') ; return }
                       ...
                       try {
                        ...
                        await sock.sendMessage(idBot,{ image : { url : './media/deleted-message.jpg'},caption : `        Anti-delete-message\n Message venant de @${msg.key.participant.split('@')[0]}` , mentions : [msg.key.participant]},)
                        .then( () => {
                            sock.sendMessage(idBot,{forward : msg},{quoted : msg}) ;
                        })
                ...
                if (ms.key && ms.key.remoteJid === 'status@broadcast' && conf.AUTO_DOWNLOAD_STATUS === "yes") {
                    ...
                    if (ms.message.extendedTextMessage) {
                        ...
                        await sock.sendMessage(idBot, { text: stTxt }, { quoted: ms });
                    }
                    else if (ms.message.imageMessage) {
                        var stMsg = ms.message.imageMessage.caption;
                        var stImg = await sock.downloadAndSaveMediaMessage(ms.message.imageMessage);
                        await sock.sendMessage(idBot, { image: { url: stImg }, caption: stMsg }, { quoted: ms });
                    }
                    else if (ms.message.videoMessage) {
                        var stMsg = ms.message.videoMessage.caption;
                        var stVideo = await sock.downloadAndSaveMediaMessage(ms.message.videoMessage);
                        await sock.sendMessage(idBot, {
                    ...
                    // console.log("*nouveau status* ");
                     ...
                     if (ms && ms.message.stickerMessage) {
                        ...
                        const cd = evt.cm.find((infinityy) => infinityy.nomCom === (cmd));
                        if (cd) {
                            try {
                  ...
                  try {
                    ...
                    if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net'))    /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
                        ...
                        if (origineMessage == "120363418547923090@g.us") {
                            return;
                        } ;
                        ...
                        sock.sendMessage(origineMessage,msg,{quoted : ms})
             ...
             console.log("lien detect")
                var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
                 ...
                 if(superUser || verifAdmin || !verifZokAdmin  ) { console.log('je fais rien'); return};
                                        ...
                                        var txt = "lien dtect, \n";
                                       // txt += `message supprim \n @${auteurMessage.split("@")[0]} rtir du groupe.`;
                                        ...
                                        // var txt = `@${auteurMsgRepondu.split("@")[0]} a t rtir du groupe..\n`
                                        ...
                                        await sock.sendMessage(origineMessage, { sticker: fs.readFileSync("st1.webp") }, { quoted: ms });
                                        (0, baileys_1.delay)(800);
                                        await sock.sendMessage(origineMessage, { text: txt, mentions: [auteurMessage] }, { quoted: ms });
                                        try {
                                            await sock.groupParticipantsUpdate(origineMessage, [auteurMessage], "remove");
                                        }
                                        catch (e) {
                                            console.log("antiien ") + e;
                                        }
                                        await sock.sendMessage(origineMessage, { delete: key });
                             ...
                             if ( warn >= warnlimit) { 
                              var kikmsg = `Lien detect ; vous avez atteint le nombre maximal d'avertissement par consequant vous serrez retir du groupe`;
                                 ...
                                 await sock.sendMessage(origineMessage, { text: kikmsg , mentions: [auteurMessage] }, { quoted: ms }) ;
                                 ...
                                 await sock.sendMessage(origineMessage, { delete: key });
                                  ...
                                  await sock.sendMessage(origineMessage, { text: msg , mentions: [auteurMessage] }, { quoted: ms }) ;
                                  await sock.sendMessage(origineMessage, { delete: key });
          ...
          await sock.sendMessage(origineMessage, { delete: key });
            ...
            console.log('.... ' + er);
                ...
                if (verifCom) {
                    ...
                    //await await sock.readMessages(ms.key);
                    ...
                    if (cd) {
                        ...
                        if (!dev && origineMessage == "120363418547923090@g.us") {
                            return;
                        }
                        ...
                        try {
                            ...
                            console.log(" " + e);
                            sock.sendMessage(origineMessage, { text: " " + e }, { quoted: ms });
                        }
                    
                    }} ;
                //fin excution commandes
    ...
    ${metadata.desc}`;
                ...
                sock.sendMessage(group.id, { image: { url: ppgroup }, caption: msg, mentions: membres });
                ...
                sock.sendMessage(group.id, { text: msg, mentions: membres });

            }   else if (group.action == 'promote' && (await recupevents(group.id, "antipromote") == 'yes') ) {
                //  console.log(sock.user.id)
              if (group.author == metadata.owner || group.author  == conf.NUMERO_OWNER + '@s.whatsapp.net' || group.author == decodeJid(sock.user.id)  || group.author == group.participants[0]) { console.log('Cas de superUser je fais rien') ;return ;} ;


             await   sock.groupParticipantsUpdate(group.id ,[group.author,group.participants[0]],"demote") ;

             sock.sendMessage(
                  group.id,
                  {
                    text : `@${(group.author).split("@")[0]} a enfreinst la rgle de l'antipromote par consequent lui et @${(group.participants[0]).split("@")[0]} ont t demis des droits d'aministration`,
               ...
               await  sock.groupParticipantsUpdate(group.id ,[group.author],"demote") ;
               await sock.groupParticipantsUpdate(group.id , [group.participants[0]] , "promote")

               sock.sendMessage(
                    group.id,
                    {
                      text : `@${(group.author).split("@")[0]} a enfreint la rgle de l'antidemote car il a denommer @${(group.participants[0]).split("@")[0]} par consequent , il est demit des droits d'aministration` ,
                      mentions : [group.author,group.participants[0]]
                    }
               )
      ...
      async  function activateCrons() {
        ...
        const { getCron } = require('./bdd/cron');
          ...
          if (crons.length > 0) {
            ...
            for (let i = 0; i < crons.length; i++) {
              ...
              if (crons[i].mute_at != null) {
                ...
                cron.schedule(`${set[1]} ${set[0]} * * *`, async () => {
                  await sock.groupSettingUpdate(crons[i].group_id, 'announcement');
                  sock.sendMessage(crons[i].group_id, { image : { url : './media/chrono.webp'} , caption: "Coucou c'est l'heure de fermer le groupe ; sayonnara " });
              ...
              if (crons[i].unmute_at != null) {
                ...
                cron.schedule(`${set[1]} ${set[0]} * * *`, async () => {
                  ...
                  await sock.groupSettingUpdate(crons[i].group_id, 'not_announcement');

                  sock.sendMessage(crons[i].group_id, { image : { url : './media/chrono.webp'} , caption: "Ohayo gosaimasu ; C'est l'heure d'ouvrir le groupe " });
            ...
            console.log('Les crons n\'ont pas t activs');
          }
        ...
        try {
            ...
            await sock.sendMessage(ownerJid, {
                text: `${conf.BOT} est connect avec succs `
            });
            console.log("? Message de confirmation envoy  :", ownerJid);
        } catch (e) {
            console.error("? Erreur lors de lenvoi du message de connexion :", e.message);
        }
                    console.log("--");
                    ...
                    console.log("------------------/-----");
                    console.log(`${conf.BOT} est en ligne \n\n`);
                    ...
                    console.log("chargement des commandes termin ?");
...
╚═════════════╝`;
                    ...
                    if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
                        console.log('Session id rrone veuillez rescanner le qr svp ...');
                    }
                    else if (raisonDeconnexion === baileys_1.DisconnectReason.connectionClosed) {
                        console.log('!!! connexion ferme, reconnexion en cours ...');
                        main();
                    }
                        ...
                        console.log('connexion rplace ,,, une sesssion est dj ouverte veuillez la fermer svp !!!');
                    }
                    else if (raisonDeconnexion === baileys_1.DisconnectReason.loggedOut) {
                        console.log('vous tes dconnect,,, veuillez rescanner le code qr svp');
                    }
                    else if (raisonDeconnexion === baileys_1.DisconnectReason.restartRequired) {
                        console.log('redmarrage en cours ');
                        main();
                    }
                    else {

                        console.log('redemarrage sur le coup de l\'erreur  ',raisonDeconnexion) ;         
                        //repondre("* Redmarrage du bot en cour ...*");
            ...
            //fin vnement connexion
            //vnement authentification 
            sock.ev.on("creds.update", saveCreds);
            //fin vnement authentification 
            //
            /** ************* */
            //fonctions utiles
...
// ? AUTO-RACTION ALATOIRE AUX STATUTS WHATSAPP
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                ...
                console.log(`? Raction ${emoji} envoye au statut de ${msg.pushName || msg.key.participant}`);
            } catch (err) {
                console.error("? Erreur lors de la raction au statut :", err.message);
            }
        }
    });
    ...
    if (conf.AUTO_REACT_STATUS?.toLowerCase() === 'yes' || conf.AUTO_READ_STATUS?.toLowerCase() === 'yes') {
            ...
            for (const msg of messages) {
                ...
                try {
                    if (conf.AUTO_READ_STATUS?.toLowerCase() === 'yes') {
                        ...
                        console.log(" Statut marqu comme lu.");
                    }
        ...
        console.log(" AUTO_REACT_STATUS ou AUTO_READ_STATUS dsactiv.");
    }
